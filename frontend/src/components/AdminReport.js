...
const AdminReport = () => {
  ...
  const generatePDF = () => {
    ...
    if (report) {
      ...
      doc.text(`Total Añadidos al Carrito: ${report.totalAddToCartCount}`, 10, 50);
      ...
      report.mostViewedProducts.forEach((product, index) => {
        doc.setFontSize(10);
        doc.text(`${index + 1}. ${product.name} - Vistas: ${product.views}`, 15, 70 + index * 10);
      });
    ...
    <div className="container mt-5">
      ...
      {report && (
        <div>
          ...
          <p>
            <strong>Total Añadidos al Carrito:</strong> {report.totalAddToCartCount}
          </p>
